...
#' @param title vannomap title. Default: NULL
...
vannomap = function(X,
    ...
    Gs = sapply(2:n, function(i) 
                {
                    ...
                    G = ggplot2::ggplot(X, aes(y = as.numeric(x), fill = fill, x = 1)) +
                        ggplot2::geom_tile(col =tile.col) +
                    ...
                    G = G + ggplot2::theme(aspect.ratio = ratio,
                            ...
                            ggplot2::labs(y = x.name, x = cols[i])
    ...
    patchwork::wrap_plots(Gs, guides = guides, nrow = nrow, heights = heights, ...)
}
