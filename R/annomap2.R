...
annomap = function(X,
    ...
    if (is.null(cols)) {
        cols = brewerland::colourScale(X$fill)
    }

#     if (is.numeric(X$fill)) scale_fill = ggplot2::scale_fill_gradient
#     else scale_fill = ggplot2::scale_fill_manual

    # plot
    G = G + ggplot2::geom_tile(col = tile.col, size = tile.size) + 
            ggplot2::theme_bw() +
...
#             scale_fill(cols) +
    ...
    if (is.numeric(X$fill)) G = G + ggplot2::scale_fill_gradientn(colours = cols)
    else G = G + ggplot2::scale_fill_manual(values = cols)
